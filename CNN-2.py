...
filter_size = 3
pool_size , stride = 4 , 4
num_filters = 1


dim_cl =  ( X_train.shape[0] , num_filters,  X_train.shape[2] - filter_size + 1 , X_train.shape[3] - filter_size + 1 )
dim_max = ( dim_cl[0] , dim_cl[1],  ((dim_cl[2] - pool_size)//stride) +1 , ((dim_cl[3] - pool_size)//stride) +1 )
dim_cl2 =  ( X_train.shape[0] , num_filters,  dim_max[2] - filter_size + 1 , dim_max[3] - filter_size + 1 )
dim_max2 = ( dim_cl2[0] , dim_cl2[1],  ((dim_cl2[2] - pool_size)//stride) +1 , ((dim_cl2[3] - pool_size)//stride) +1 )
dim_flat = (X_train.shape[0], dim_max2[2]* dim_max2[3] * num_filters)
print( dim_cl  , dim_max ,dim_cl2, dim_max2 , dim_flat)
...
conv_layer2 = ConvolutionalLayer(input_channels=1, 
                                num_filters = num_filters,
                                kernel_size= filter_size, 
                                stride=1)
#conv_layer2.setWeights(weights)
relu_layer2 = ReLuLayer()

maxpool2 = MaxPoolingLayer(pool_size= pool_size, stride=stride)
...
for epoch in range(epochs):
    ...
    for layer in reversed(layers):
        #print(f"---------------- backward {layer.__class__.__name__} ------- ")
